...
const AppHeader = () => {
  ...
  return (
    <div
      className={`w-screen h-[10%] flex md:justify-center justify-between items-center bg-${
        scrollingUp ? "white" : "black"
      } relative`}
    >
      ...
      <span className="sm:hidden flex w-[100%] justify-between items-center px-5 sm:p-10">
        <div
          className={`flex items-baseline text-${
            scrollingUp ? "black" : "white"
          }`}
        >
        ...
        <span onClick={() => setMenu(!menu)} className="z-40">
          <box-icon name={menu ? "x" : "menu"} color="#ffffff"></box-icon>
        </span>
        <div
          className={`w-screen h-screen flex justify-center flex-col items-center gap-5 font-thin absolute left-0 top-0 z-10 bg-black ${
            menu ? "" : "hidden"
          } bg-opacity-75`}
        >
          <Link
